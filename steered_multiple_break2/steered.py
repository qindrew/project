...
#%%
def generate_context(args="", script="run.in", store_freq=300):
    """
...
def steer(argv):
    args = get_args(argv)
    m, tri_idxs, blocked_idxs, type_sets, atoms, initial_positions = load_model(filepath=args.filepath, subset=args.subset)
    context_args = {"store_freq": args.log_steps}
    context_args["args"] = "-k on g 1 -sf kk -pk kokkos neigh half newton on"

    cvs = [Distance([14, 17]),Distance([52, 55]),Distance([90, 93]),Distance([128, 131]),]

    starting_CV = np.asarray([1.5,1.5,1.5,1.5])
    time_steps = args.time_steps
    ending_CV = np.asarray([5,5,5,5])
    velocity = (ending_CV-starting_CV) / time_steps / 0.2
    callback = SteeredLogger('log.steered', args.log_steps)
    method = Steered(cvs, np.asarray([10,10]), starting_CV, velocity)
